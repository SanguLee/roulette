...
function updateData() {
	...
	$('li.item:not(.empty)').each(function(i, e) {
		...
		if(!data.getGroupItems.length>i) data.addItem(data.nowGroupIndex, title);
		else data.setItemTitle(data.nowGroupIndex, i, title);
	
		var weight = parseFloat($e.find('.item-weight').attr('data-weight')) || 0;
		var chance = weight / weightSum;
		$e.attr('data-chance', (chance * 100).toFixed(0));
...
function lock(b) {
	var $g = $('#groupSelect, #groupEdit .group-btn, #fileButton, .item.empty');
	if (b) {
